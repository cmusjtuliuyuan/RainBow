...
class DQNAgent:
    ...
    def __init__(self,
        ...
        self._beta = BETA_BEGIN
        self._beta_increment = (BETA_END-BETA_BEGIN)/400000.0
        self._epsilon = EPSILON_BEGIN if is_noisy==0 else 0.
        self._epsilon_increment = (EPSILON_END - EPSILON_BEGIN)/400000.0 if is_noisy==0 else 0.
    ...
    def select_action(self, sess, state, epsilon, model):
        ...
        if np.random.rand() < epsilon:
            ...
            state = Preprocessor.state2float(state)
    ...
    def get_mean_max_Q(self, sess, samples):
        ...
        for i in range(0, len(samples), INCREMENT):
            feed_dict = {self._online_model['input_frames']:
                Preprocessor.state2float(samples[i: i + INCREMENT])}
    ...
    def evaluate(self, sess, env, num_episode):
        ...
        while num_finished_episode < num_episode:
            ...
            next_action = self.select_action(sess, current_state, 0, self._online_model)
    ...
    def get_multi_step_sample(self, env, sess, num_step, epsilon):
        current_state = env.get_state()
        next_action = self.select_action(sess, current_state, epsilon, self._online_model)
        env.take_action(next_action)
        old_state, action, reward, new_state, is_terminal = env.get_train_tuple()
    ...
    def fit(self, sess, env, num_iterations, do_train=True):
        ...
        for t in range(0, num_iterations):
            ...
            if do_train and t%self._update_freq==0:
                if self._is_per==1:
                ...
                feed_dict = {self._target_model['input_frames']: Preprocessor.state2float(new_state_list),
                             self._online_model['input_frames']: Preprocessor.state2float(old_state_list),
                ...
                if self._is_double_dqn:
                    action_chosen_by_online = sess.run(self._online_model['action'], feed_dict={
                                self._online_model['input_frames']: Preprocessor.state2float(new_state_list)})
    ...
    def _get_error(self, sess, old_state, action, reward, new_state, is_terminal):
        ...
        '''
        feed_dict = {self._target_model['input_frames']: Preprocessor.state2float(new_state),
                     self._online_model['input_frames']: Preprocessor.state2float(old_state),
